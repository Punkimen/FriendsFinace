...
if ($(window).width() <= '750') {
    $('.mainMenu__list-link').on('click', function () {
...
let tabs = $('.profile-head__tabs-item');
...
if ($(window).width() <= '678') {
    $(window).scroll(function () {
        if ($(window).scrollTop() > 350) {
            $('.top__credit-btn').fadeIn(500)
        } else {
            $('.top__credit-btn').fadeOut(300)
        }
    });
    $('.top__credit-btn').on('click', function () {
        $("html, body").animate({
            "scrollTop": 0,
        }, 1000)
        return false;
...
if ($(window).width() <= '678') {
    $('.arrage__credit-link').css('display', 'block')
}
...
const adaptiveHeight = () => {
    let calcHeight = $('.calc__container').height();
    $('.slider__content').css('height', calcHeight + 'px')
    console.log(calcHeight);
    console.log($('.slider__content').height());
}
if ($(window).width() <= '650') {
    adaptiveHeight()
} else {
    $('.slider__content').css('height', "auto")
}
...
$(window).resize(function () {
    ...
    if (windowWidth <= '678') {
        ...
        $('.arrage__credit-link').css('display', 'none')
    }
    if (windowWidth <= '750') {
